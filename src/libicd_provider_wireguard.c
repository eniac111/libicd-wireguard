...
#define WP_DEBUG(fmt, ...) ILOG_DEBUG(("[WIREGUARD PROVIDER] "fmt), ##__VA_ARGS__)
#define WP_INFO(fmt, ...) ILOG_INFO(("[WIREGUARD PROVIDER] " fmt), ##__VA_ARGS__)
#define WP_WARN(fmt, ...) ILOG_WARN(("[WIREGUARD PROVIDER] %s.%d:" fmt), __func__, __LINE__, ##__VA_ARGS__)
#define WP_ERR(fmt, ...) ILOG_ERR(("[WIREGUARD PROVIDER] %s.%d:" fmt), __func__, __LINE__, ##__VA_ARGS__)
#define WP_CRIT(fmt, ...) ILOG_CRIT(("[WIREGUARD PROVIDER] %s.%d:" fmt), __func__, __LINE__, ##__VA_ARGS__)

struct _provider_wireguard_private {
	/* For pid monitoring */
...
typedef struct _provider_wireguard_private provider_wireguard_private;

#define PROVIDER_WIREGUARD_STATE_NONE 0
#define PROVIDER_WIREGUARD_STATE_STOPPED 1
#define PROVIDER_WIREGUARD_STATE_STARTED 2
#define PROVIDER_WIREGUARD_STATE_CONNECTED 3

struct _wireguard_network_data {
	provider_wireguard_private *private;
...
static wireguard_network_data *icd_wireguard_find_first_network_data(provider_wireguard_private * private)
{
	GSList *l;
	...
	for (l = private->network_data_list; l; l = l->next) {
		...
		if (!found)
			WP_WARN("wireguard network data is NULL");
		else {
			return found;
		}
	}
...
static wireguard_network_data *icd_wireguard_find_network_data(const gchar * network_type, guint network_attrs,
						   const gchar * network_id, provider_wireguard_private * private)
{
	GSList *l;
...
static void wireguard_get_stop_reply(DBusPendingCall * pending, gpointer user_data)
{
	/* We don't care */
	return;
}

static void network_free_all(wireguard_network_data * network_data)
{
	provider_wireguard_private *priv = network_data->private;
...
static void network_stop_all(wireguard_network_data * network_data)
{
	DBusMessage *msg;
	msg = dbus_message_new_method_call(ICD_WIREGUARD_DBUS_INTERFACE, ICD_WIREGUARD_DBUS_PATH, ICD_WIREGUARD_DBUS_INTERFACE, "Stop");

	if (icd_dbus_send_system_mcall(msg, -1, wireguard_get_stop_reply, network_data) == FALSE) {
		/* Call down callback right away */
		WP_WARN("icd_dbus_send_system_msg failed when requesting Stop");
...
static void wireguard_get_start_reply(DBusPendingCall * pending, gpointer user_data)
{
	DBusMessage *message;
	int reply = 0;
	wireguard_network_data *network_data = user_data;
	...
	if (reply != WIREGUARD_DBUS_METHOD_START_RESULT_OK) {
...
wireguard_provider_statuschanged_sig(DBusConnection * connection, DBusMessage * message, void *user_data)
{
	provider_wireguard_private *priv = user_data;

	if (dbus_message_is_signal(message, ICD_WIREGUARD_DBUS_INTERFACE, ICD_WIREGUARD_SIGNAL_STATUSCHANGED)) {
		const char *status = NULL;
		const char *mode = NULL;
		int new_state = PROVIDER_WIREGUARD_STATE_NONE;

		if (!dbus_message_get_args(message, NULL,
					   DBUS_TYPE_STRING, &status, DBUS_TYPE_STRING, &mode, DBUS_TYPE_INVALID)) {
			WP_WARN("Unable to parse arguments of " ICD_WIREGUARD_SIGNAL_STATUSCHANGED);
		}

		/* Find network data, check status, potentially call callbacks based on
		 * state */
		wireguard_network_data *network_data = icd_wireguard_find_first_network_data(priv);
		...
		if (strcmp(status, ICD_WIREGUARD_SIGNALS_STATUS_STATE_STOPPED) == 0) {
			WP_DEBUG("New state: Stopped");
			new_state = PROVIDER_WIREGUARD_STATE_STOPPED;
		} else if (strcmp(status, ICD_WIREGUARD_SIGNALS_STATUS_STATE_STARTED) == 0) {
			WP_DEBUG("New state: Started");
			new_state = PROVIDER_WIREGUARD_STATE_STARTED;
		} else if (strcmp(status, ICD_WIREGUARD_SIGNALS_STATUS_STATE_CONNECTED) == 0) {
			WP_DEBUG("New state: Connected");
			new_state = PROVIDER_WIREGUARD_STATE_CONNECTED;
		}
		...
		if (new_state > network_data->state) {
			if (new_state == PROVIDER_WIREGUARD_STATE_CONNECTED) {
 ...
 */
static void wireguard_connect(const gchar * service_type,
...
{
	provider_wireguard_private *priv = *private;
	WP_DEBUG("wireguard_connect: %s\n", network_id);
	...
	network_data->state = PROVIDER_WIREGUARD_STATE_STOPPED;

	/* Issue dbus call, and upon dbus call result, call the connect_cb */

	DBusMessage *msg;
	msg = dbus_message_new_method_call(ICD_WIREGUARD_DBUS_INTERFACE, ICD_WIREGUARD_DBUS_PATH, ICD_WIREGUARD_DBUS_INTERFACE, "Start");
	dbus_message_append_args(msg, DBUS_TYPE_STRING, &service_id, DBUS_TYPE_INVALID);

	if (icd_dbus_send_system_mcall(msg, -1, wireguard_get_start_reply, network_data) == FALSE) {
		/* Call down callback right away */
		WP_WARN("icd_dbus_send_system_msg failed when requesting Start");
		dbus_message_unref(msg);
 ...
 * @param private              reference to the private icd_srv_api member
 */
static void wireguard_disconnect(const gchar * service_type,
			   const guint service_attrs,
			   const gchar * service_id,
			   const gchar * network_type,
			   const guint network_attrs,
...
{
	WP_DEBUG("wireguard_disconnect: %s\n", network_id);
	provider_wireguard_private *priv = *private;

	wireguard_network_data *network_data = icd_wireguard_find_network_data(network_type, network_attrs, network_id, priv);
 ...
 */
static void wireguard_identify(enum icd_scan_status status,
			 const gchar * network_type,
...
{
	WP_DEBUG("wireguard_identify: network_type: %s, network_name: %s, network_id: %s\n", network_type, network_name,
		 network_id);

	char *gconf_service_id = NULL;
	gboolean match = FALSE;

	match = network_is_wireguard_provider(network_id, &gconf_service_id);

	/* We construct a name here to make it apparent this is a wireguard provider */
	gchar *name = g_strconcat(network_name, " (", WIREGUARD_PROVIDER_NAME, ") ", NULL);
	WP_DEBUG("wireguard_identify: called for: %s\n", name);

	if (match) {
		identify_cb(ICD_SRV_IDENTIFIED, WIREGUARD_PROVIDER_TYPE,	/* service type */
			    name,
			    WIREGUARD_DEFAULT_SERVICE_ATTRIBUTES,
			    gconf_service_id,
			    WIREGUARD_DEFAULT_SERVICE_PRIORITY, network_type, network_attrs, network_id, identify_cb_token);

	} else {
		/* XXX: Do we really need to add provider type and provider id when we
		 * don't match it? */
		identify_cb(ICD_SRV_UNKNOWN, WIREGUARD_PROVIDER_TYPE,	/* service type */
			    name,
			    WIREGUARD_DEFAULT_SERVICE_ATTRIBUTES,
			    gconf_service_id,
			    WIREGUARD_DEFAULT_SERVICE_PRIORITY, network_type, network_attrs, network_id, identify_cb_token);
 ...
 */
static void wireguard_srv_destruct(gpointer * private)
{
	provider_wireguard_private *priv = *private;

	WP_DEBUG("wireguard_srv_destruct: priv %p\n", priv);

	icd_dbus_disconnect_system_bcast_signal(ICD_WIREGUARD_DBUS_INTERFACE, wireguard_provider_statuschanged_sig, priv,
						ICD_WIREGUARD_SIGNAL_STATUSCHANGED_FILTER);

	wireguard_network_data *data = NULL;
	while (data = icd_wireguard_find_first_network_data(priv), data != NULL) {
...
gboolean icd_srv_init(struct icd_srv_api * srv_api,
		      ...
		      gpointer watch_cb_token, icd_srv_close_fn close, icd_srv_limited_conn_fn limited_conn)
{
	provider_wireguard_private *priv = g_new0(provider_wireguard_private, 1);
	WP_DEBUG("icd_srv_init\n");
	srv_api->version = ICD_SRV_MODULE_VERSION;
	srv_api->private = priv;
	srv_api->connect = wireguard_connect;
	srv_api->disconnect = wireguard_disconnect;
	srv_api->identify = wireguard_identify;
	srv_api->srv_destruct = wireguard_srv_destruct;
	...
	if (!icd_dbus_connect_system_bcast_signal
	    (ICD_WIREGUARD_DBUS_INTERFACE, wireguard_provider_statuschanged_sig, priv, ICD_WIREGUARD_SIGNAL_STATUSCHANGED_FILTER)) {
		WP_ERR("Unable to listen to icd2 wireguard signals");
		g_free(priv);
		return FALSE;
	}
